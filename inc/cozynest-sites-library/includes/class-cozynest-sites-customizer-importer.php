...
/**
 * Cozynest Sites Customizer Importer
 */
class Cozynest_Sites_Customizer_Importer {
	/**
	...
	public static function import( $customizer_import_file_path ) {
		...
		Cozynest_Sites_Logger::get_instance()->info( __( 'The customizer import has finished successfully', 'cozynest' ) );
		return true;
	}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					/* Translators: import*/
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'cozynest' ),
					$import_file_path
				)
			);
		}
		...
		// Get the upload data.
		$raw = wp_remote_get( $import_file_path );

		// Make sure we got the data.
		if ( empty( $raw ) ) {
			return new \WP_Error(
				'customizer_import_data_missing_content',
				esc_html__( 'Error: The customizer import file does not have any content in it. Please make sure to use the correct customizer import file.', 'cozynest' )
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'cozynest' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'cozynest' )
			);
		}
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new Cozynest_Sites_Customizer_Option(
					$wp_customize,
					$option_key,
					array(
						'default'    => '',
		...
		if ( $use_wp_customize_save_hooks ) {
			 ...
			 * @since 1.0.0
			 *
			 * @param Should the customizer import use the WP customize_save* hooks?
			 */
		...
		foreach ( $data['mods'] as $key => $val ) {
			if ( $use_wp_customize_save_hooks ) {
				 ...
				 * @since 1.0.0
				 *
				 * @param Loop through the mods and save the mods
				 */
		...
		if ( $use_wp_customize_save_hooks ) {
			/**
			 * Should the customizer import use the WP customize_save after
			 *
			 * @since 1.0.0
			 *
			 * @param Should the customizer import use the WP customize_save after
			 */
