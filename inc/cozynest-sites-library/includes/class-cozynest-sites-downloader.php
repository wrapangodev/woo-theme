...
/**
 * Cozynest Sites Downloader
 */
class Cozynest_Sites_Downloader {
	/**
	...
	public function download_file( $url, $filename ) {
		...
		if ( is_wp_error( $content ) ) {
			Cozynest_Sites_Logger::get_instance()->error(
		...
		Cozynest_Sites_Logger::get_instance()->error(
			__( 'The file was not able to save to disk, while trying to download it', 'cozynest' ),
			array(
				'url'      => $url,
				'filename' => $filename,
			)
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'cozynest' )
			);
		}
		...
		$response = wp_remote_get(
			 ...
			 * @since 1.0.0
			 *
			 * @param Get file content from the server
			 */
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					/* translators: 1: New line break, 2: Learn More link, 3: osdffk, 4: osdfk, 5: okdfs, 6: oksdfsdf. */
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'cozynest' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			 ...
			 * @since 1.0.0
			 *
			 * @param Set download_directory_path attribute
			 */
