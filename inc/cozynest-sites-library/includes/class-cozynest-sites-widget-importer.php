...
/**
 * Cozynest Sites Widget Importer
 */
class Cozynest_Sites_Widget_Importer {
	/**
	...
	public static function import( $widget_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			Cozynest_Sites_Logger::get_instance()->error( $results->get_error_message() );
		...
		Cozynest_Sites_Logger::get_instance()->debug( $message );
	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import file could not be found.', 'cozynest' )
			);
		}
		...
		if ( empty( $data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import file does not have any content in it.', 'cozynest' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import data could not be read. Please try a different file.', 'cozynest' )
			);
		}
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = __( 'Sidebar does not exist in theme (moving widget to Inactive)', 'cozynest' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'Site does not support widget', 'cozynest' ); // Explain why widget not imported.
				}
				 ...
				 * @since 1.0.0
				 *
				 * @param Filter to modify settings object before conversion to array and import.
				 */
				...
				/**
				 * Filter to modify settings arra
				 *
				 * @since 1.0.0
				 *
				 * @param Filter to modify settings arra.
				 */
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets, true ) && (array) $widget === $check_widget ) {
							...
							$widget_message      = __( 'Widget already exists', 'cozynest' ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					/**
					 * Merlin widget importer after single widget import
					 *
					 * @since 1.0.0
					 *
					 * @param Merlin widget importer after single widget import.
					 */
					...
					if ( $sidebar_available ) {
						...
						$widget_message      = __( 'Imported', 'cozynest' );
					} else {
						$widget_message_type = 'warning';
						$widget_message      = __( 'Imported to Inactive', 'cozynest' );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( 'No Title', 'cozynest' ); // Show "No Title" if widget instance is untitled.
	...
	private static function unset_default_widgets() {
		 ...
		 * @since 1.0.0
		 *
		 * @param Remove widgets from sidebars,
		 */
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'cozynest' );
		}
