...
class Cozynest_Sites {
	 ...
	 *
	 * @var string $cozynest_sites_url
	 */
	protected $cozynest_sites_url = null;
	...
	public function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args(
			$config,
			array(
				'cozynest_sites_url'   => 'cozynest-sites',
		...
		$this->cozynest_sites_url     = $config['cozynest_sites_url'];
		...
		if ( true !== $this->dev_mode ) {
			...
			$already_setup = get_option( 'cozynest_sites_' . $this->slug . '_completed' );
		...
		// Get TGMPA.
		require_once COZYNEST_SITES_DIR . 'includes/class-tgm-plugin-activation.php';
		require_once COZYNEST_SITES_DIR . 'includes/plugin.php';

		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->tgmpa = isset( $GLOBALS['tgmpa'] ) ? $GLOBALS['tgmpa'] : TGM_Plugin_Activation::get_instance();
		}

		add_action( 'admin_enqueue_scripts', array( $this, 'cozynest_sites_admin_template' ) );
		add_action( 'admin_init', array( $this, 'cozynest_sites_disable_default_elementor_color_font' ) );
		add_action( 'admin_init', array( $this, 'cozynest_sites_required_classes' ) );
		add_action( 'admin_init', array( $this, 'cozynest_sites_redirect' ), 30 );
		add_action( 'admin_init', array( $this, 'cozynest_sites_steps' ), 30, 0 );
		add_action( 'merlin_admin_head', array( $this, 'cozynest_sites_css' ) );
		add_action( 'admin_menu', array( $this, 'cozynest_sites_add_admin_menu' ) );
		add_action( 'admin_init', array( $this, 'cozynest_sites_admin_page' ), 30, 0 );
		add_action( 'admin_init', array( $this, 'cozynest_sites_ignore' ), 5 );
		add_action( 'admin_footer', array( $this, 'cozynest_sites_svg_sprite' ) );
		add_filter( 'tgmpa_load', array( $this, 'cozynest_sites_load_tgmpa' ), 10, 1 );
		add_action( 'tgmpa_register', array( $this, 'cozynest_sites_register_required_plugins' ), 10, 0 );
		add_action( 'admin_init', array( $this, 'cozynest_sites_register_required_plugins' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_content', array( $this, 'cozynest_sites_ajax_content' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_site_filter_demo', array( $this, 'cozynest_site_filter_demo' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_load_more_demo', array( $this, 'cozynest_sites_load_more_demo' ), 10, 0 );
		add_action( 'init', array( $this, 'cozynest_sites_set_cookie' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_get_total_content_import_items', array( $this, 'cozynest_sites_ajax_get_total_content_import_items' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_update_selected_import_data_info', array( $this, 'cozynest_sites_update_selected_import_data_info' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_plugins', array( $this, 'cozynest_sites_ajax_plugins' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_child_theme', array( $this, 'cozynest_sites_generate_child' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_activate_license', array( $this, 'cozynest_sites_ajax_activate_license' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_selected_import_data_info', array( $this, 'cozynest_sites_selected_import_data_info' ), 10, 0 );
		add_action( 'wp_ajax_cozynest_sites_import_finished', array( $this, 'cozynest_sites_import_finished' ), 10, 0 );
		add_filter( 'pt-importer/new_ajax_request_response_data', array( $this, 'cozynest_sites_pt_importer_new_ajax_request_response_data' ) );
		add_action( 'import_start', array( $this, 'cozynest_sites_before_content_import_setup' ) );
		add_action( 'cozynest_sites_after_all_import', array( $this, 'cozynest_sites_after_content_import_setup' ) );
		add_action( 'admin_init', array( $this, 'cozynest_sites_register_import_files' ) );
		add_action( 'wp_ajax_cozynest_sites_feature_activated', array( $this, 'cozynest_ajax_all_feature_activated' ) );
		add_action( 'wp_ajax_cozynest_sites_module_action', array( $this, 'cozynest_ajax_module_action' ) );
	 ...
	 * @return void
	 */
	public function cozynest_sites_disable_default_elementor_color_font() {
	 ...
	 */
	public function cozynest_sites_required_classes() {
		if ( ! class_exists( '\WP_Importer' ) ) {
			require ABSPATH . '/wp-admin/includes/class-wp-importer.php';
		}

		require_once COZYNEST_SITES_DIR . 'includes/class-cozynest-sites-downloader.php';

		$this->importer = new ProteusThemes\WPContentImporter2\Importer( array( 'fetch_attachments' => true ) );

		require_once COZYNEST_SITES_DIR . 'includes/class-cozynest-sites-widget-importer.php';

		if ( ! class_exists( 'WP_Customize_Setting' ) ) {
			require_once ABSPATH . 'wp-includes/class-wp-customize-setting.php';
		}

		require_once COZYNEST_SITES_DIR . 'includes/class-cozynest-sites-customizer-option.php';
		require_once COZYNEST_SITES_DIR . 'includes/class-cozynest-sites-customizer-importer.php';
		require_once COZYNEST_SITES_DIR . 'includes/class-cozynest-sites-redux-importer.php';
		require_once COZYNEST_SITES_DIR . 'includes/class-cozynest-sites-hooks.php';
		require_once COZYNEST_SITES_DIR . 'includes/admin/class-admin.php';

		$this->hooks = new Cozynest_Sites_Hooks();
	...
	public function switch_theme() {
		if ( ! is_child_theme() ) {
			set_transient( $this->theme->template . '_cozynest_sites_redirect', 1 );
		}
	}
	...
	public function index_data( $post_id, $data, $meta, $comments, $terms ) {
		...
		if ( ! class_exists( 'Cozynest_Pro' ) ) {
			return 'failed';
		}

		$index = new Cozynest_Pro();
	 ...
	 */
	public function cozynest_sites_redirect() {

		if ( ! get_transient( $this->theme->template . '_cozynest_sites_redirect' ) ) {
			return;
		}

		delete_transient( $this->theme->template . '_cozynest_sites_redirect' );

		wp_safe_redirect( admin_url( 'themes.php?page= ' . $this->cozynest_sites_url ) );

		exit;
	}

	/**
	 * Give the user the ability to ignore Cozynest Sites.
	 */
	public function cozynest_sites_ignore() {

		// Bail out if not on correct page.
		if ( ! isset( $_GET['_wpnonce'] ) || ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'cozynest-sites-ignore-nonce' ) || ! is_admin() || ! isset( $_GET[ $this->ignore ] ) || ! current_user_can( 'manage_options' ) ) ) {
			return;
		}
	 ...
	 */
	public function cozynest_sites_load_tgmpa( $status ) {
	 ...
	 */
	public function cozynest_sites_add_admin_menu() {
		...
		$this->hook_suffix = add_theme_page(
			...
			esc_html( $strings['admin-menu'] ),
			'edit_theme_options',
			$this->cozynest_sites_url,
			array( $this, 'cozynest_sites_admin_page' )
		);
	}
	 ...
	 * Add the admin page.
	 */
	public function cozynest_sites_css() {
		// Enqueue styles.
		wp_enqueue_style( 'cozynest-sites-admin-styles', get_template_directory_uri() . '/inc/cozynest-sites-library/assets/css/cozynest-sites.css', array(), '1.0.1' );
	}

	/**
	 * Add the admin page.
	 */
	public function cozynest_sites_admin_page() {

		// Strings passed in from the config file.
		$strings = $this->strings;

		// Do not proceed, if we're not on the right page.
		if ( empty( $_GET['page'] ) || $this->cozynest_sites_url !== $_GET['page'] ) {
			return;
		}
		...
		// Enqueue javascript.
		wp_enqueue_script( 'cozynest-sites-admin-scripts', get_template_directory_uri() . '/inc/cozynest-sites-library/assets/js/cozynest-sites.js', array( 'jquery' ), '1.0.0', true );

		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'cozynest' ),
		);
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			...
			wp_localize_script(
				'cozynest-sites-admin-scripts',
				'cozynest_sites_params',
				array(
					...
					'wpnonce'          => wp_create_nonce( 'cozynest_sites_nonce' ),
					'texts'            => $texts,
					'total_page'       => $total_page,
					'activate'         => __( 'Activate', 'cozynest' ),
					'activating'       => __( 'Activating...', 'cozynest' ),
					'deactivate'       => __( 'Deactivate', 'cozynest' ),
					'deactivating'     => __( 'Deactivating...', 'cozynest' ),
				)
			);
		} else {
		...
		<div class="merlin__wrapper">
			...
			<?php $ignore_url = wp_nonce_url( admin_url( '?' . $this->ignore . '=true' ), 'cozynest-sites-ignore-nonce' ); ?>
	 ...
	 * SVG
	 */
	public function cozynest_sites_svg_sprite() {

		// Define SVG sprite file.
		$svg = COZYNEST_SITES_DIR . 'assets/images/sprite.svg';

		// If it exists, include it.
		if ( file_exists( $svg ) ) {

			/**
			 * Cozynest sites svg sprite
			 *
			 * @since 2.5
			 */
			require_once apply_filters( 'cozynest_sites_svg_sprite', $svg );
		}
	}
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'cozynest' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'cozynest' );
		}
	...
	public function svg_allowed_html() {
		...
		/**
		 * Cozynest sites svg allowed html
		 *
		 * @since 2.5
		 */
		return apply_filters( 'cozynest_sites_svg_allowed_html', $array );
	}
	...
	public function loading_spinner_allowed_html() {
		...
		/**
		 * Cozynest sites loading spinner allowed html
		 *
		 * @since 2.5
		 */
		return apply_filters( 'cozynest_sites_loading_spinner_allowed_html', $array );
	}

	/**
	 * Setup steps.
	 */
	public function cozynest_sites_steps() {

		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'cozynest' ),
		...
		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'cozynest' ),
		...
		if ( ! empty( $this->import_files ) && ( count( $this->import_files ) > 1 ) ) {
		...
		// 	setcookie( 'demo', $this->import_files[0]['id'], time() + 7200 );
		// }

		// Show the plugin importer, only if TGMPA is included.
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'cozynest' ),
				'view' => array( $this, 'plugins' ),
			);
		}

		// Show the plugin importer, only if TGMPA is included.
		$modules_activated = get_option( 'cozynest_pro_fully_featured_activate' );
		if ( ! empty( $this->import_files ) && class_exists( 'Cozynest_Pro' ) && defined( 'COZYNEST_PRO_VERSION' ) && ! $modules_activated ) {
			$this->steps['modules'] = array(
				'name' => esc_html__( 'Modules', 'cozynest' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['install'] = array(
				'name' => esc_html__( 'Instal', 'cozynest' ),
				'view' => array( $this, 'install' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'cozynest' ),
			'view' => array( $this, 'ready' ),
		);

		/**
		 * Cozynest_sites_steps
		 *
		 * @since 2.5
		 */
		$this->steps = apply_filters( $this->theme->template . '_cozynest_sites_steps', $this->steps );
	}
	...
	protected function welcome() {
		...
		<footer class="merlin__content__footer">
			...
			<?php wp_nonce_field( 'cozynest-sites' ); ?>
		</footer>
	...
	protected function welcome_handler() {
		...
		check_admin_referer( 'cozynest-sites' );
	...
	private function is_theme_registered() {
		...
		/**
		 * Cozynest sites is theme registered
		 *
		 * @since 2.5
		 */
		return apply_filters( 'cozynest_sites_is_theme_registered', $is_registered );
	}
	...
	protected function child() {
		...
		$child_theme_option = get_option( 'cozynest_sites_' . $this->slug . '_child' );
	...
	protected function plugins() {
		// Variables.
		$url    = wp_nonce_url( add_query_arg( array( 'plugins' => 'go' ) ), 'cozynest-sites' );
		$method = '';
		$fields = ( isset( $_POST['plugin'] ) && wp_verify_nonce( sanitize_key($_POST['plugin'] ), 'cozynest-sites' ) ) ?  array_keys( $_POST ) : []; //phpcs:ignore
						...
						<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
							<li data-slug="<?php echo esc_attr( $slug ); ?>">
								...
								<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
									...
									<span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'cozynest' ); ?>">
										<?php esc_html_e( 'req', 'cozynest' ); ?>
									</span>
								</span>
								</label>
							</li>
	...
	protected function modules() {
		...
		$strings = $this->strings;
		// Text strings.
		$header       = __( 'Cozynest Active Module', 'cozynest' );
		$skip         = $strings['btn-skip'];
		$next         = __( 'Active Modules', 'cozynest' );
		$cozynest_pro = new Cozynest_Pro();
		$modules      = $cozynest_pro->cozynest_pro_modules();
		$nextClass    = get_option( 'cozynest_pro_fully_featured_activate' ) ? '' : 'disable';
		...
		<div class="merlin__content--transition">
			...
			<div class="step-description">
				<span class="description"><?php echo esc_html__( 'Let\'s active some modules to get your site up to speed.', 'cozynest' ); ?></span>
			</div>

			<div class="cozynest-enhance__column cozynest-pro-module">

				<a id="merlin__drawer-trigger" class="merlin__button merlin__button--knockout">
					<span><?php echo esc_html( $action ); ?></span><span class="chevron"></span>
				</a>

				<div class="cozynest-module-list">
						...
						foreach ( $modules as $k => $v ) {
							...
							<?php if ( 'activated' !== $key ) : ?>
								<li class="merlin-active-module module-item">
									<input type="checkbox" name="cozynest_module_checkbox[]" class="checkbox checkbox-content module-checkbox" value="<?php echo esc_attr( $k ); ?>" id="<?php echo esc_attr( $id ); ?>" checked>
	...
	protected function content() {
		...
		foreach ( $all_demo as $index => $demo ) {
			if ( 'elementor' == $demo['page_builder'] ) {
				$demos[] = $demo;
			}
		}
		...
		<div class="merlin__content--transition">
			...
			<div class="merlin__demos-wrapper">
				<p><?php esc_html_e( 'Select which demo data you want to import:', 'cozynest' ); ?></p>
						...
						<div class="merlin__demo">
							<div class="merlin__demo-image">
								<?php if ( ! empty( $demo_image ) ) : ?>
									<img src="<?php echo esc_url( $demo_image ); ?>" alt="<?php esc_attr_e( 'Demo Image', 'cozynest' ); ?>">
								<?php endif; ?>
							</div>
							...
							<div class="merlin__demo-content">
								...
								<div class="merlin__demo-cta">
									<a class="merlin__demo-button" href="<?php echo esc_url( $demo_preview_url ); ?>" target="_blank"><?php esc_html_e( 'Preview', 'cozynest' ); ?></a>
									<?php if ( 'pro' === $demo_type && defined( 'COZYNEST_PRO_VERSION' ) ) : ?>
										<button data-content="<?php echo esc_attr( $import_file['id'] ); ?>" class="merlin__demo-button js-select-demo"><?php esc_html_e( 'Select', 'cozynest' ); ?></button>
									<?php elseif ( 'pro' === $demo_type && ! defined( 'COZYNEST_PRO_VERSION' ) ) : ?>
										<a class="merlin__demo-button" href="https://cozynest.com/pro" target="_blank"><?php esc_html_e( 'Pricing', 'cozynest' ); ?></a>
									<?php elseif ( 'free' === $demo_type ) : ?>
										<button data-content="<?php echo esc_attr( $import_file['id'] ); ?>" class="merlin__demo-button js-select-demo" data-callback="install_contents"><?php esc_html_e( 'Select', 'cozynest' ); ?></button>
									<?php endif; ?>
								</div>
							</div>
						</div>
	...
	protected function install() {
		$import_info          = isset( $_COOKIE['demo'] ) ? $this->get_import_data_info( sanitize_text_field( wp_unslash( $_COOKIE['demo'] ) ) ) : false;
		...
		<div class="merlin__content--transition">
			...
			<p><?php echo esc_html( $paragraph ); ?></p>
			<div>
				<p><?php esc_html_e( 'Make sure you reset the site to default installation values before importing demo data.', 'cozynest' ); ?></p>
				<?php
				$link = sprintf(
					wp_kses(
						/* translators: suggested plugin link */
						__( 'You can use the <a href="%s">WP Reset</a> plugin to reset and run through demos.', 'cozynest' ),
						array(
							'a' => array(
								'href' => array(),
							),
		...
		<form action="" method="post">
			<ul class="merlin__drawer merlin__drawer--import-content js-merlin-drawer-import-content">
				<?php
				echo wp_kses(
					$this->get_import_steps_html( $import_info ),
					array(
						'li'    => array(
							'class'        => array(),
							'data-content' => array(),
	...
	protected function ready() {
		...
		update_option( 'cozynest_sites_' . $this->slug . '_completed', time() );
		?>
		...
		<div class="merlin__content--transition">
			...
			<p>
				<?php wp_kses( $paragraph, $allowed_html_array ); ?>
				<?php wp_kses( $author, $allowed_html_array ); ?>
			</p>
		</div>
	 ...
	 */
	public function cozynest_sites_generate_child() {
	 ...
	 */
	public function cozynest_sites_ajax_activate_license() {

		if ( ! check_ajax_referer( 'cozynest_sites_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'cozynest' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'cozynest' ),
				)
			);
		}
		...
		if ( ! has_filter( 'cozynest_sites_ajax_activate_license' ) ) {
			$result = $this->edd_activate_license( $license_key );
		} else {
			/**
			 * Cozynest sites ajax activate license
			 *
			 * @since 2.5
			 */
			$result = apply_filters( 'cozynest_sites_ajax_activate_license', $license_key );
		}
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'cozynest' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'cozynest' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'cozynest' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'cozynest' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'cozynest' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'cozynest' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'cozynest' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		 ...
		 * @since 2.5
		 */
		return apply_filters( 'cozynest_sites_generate_child_functions_php', $output, $slug );
	}
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$output = "
			...
			* Description: This is a child theme of {$parent}, generated by Cozynest Sites.
			* Author: {$author}
			* Template: {$slug}
			* Version: {$version}
			*/\n
		...
		/**
		 * Cozynest sites generate child style css
		 *
		 * @since 2.5
		 */
		return apply_filters( 'cozynest_sites_generate_child_style_css', $output, $slug, $parent, $version );
	}
	...
	public function generate_child_screenshot( $path ) {
		...
		/**
		 * Cozynest sites generate child screenshot
		 *
		 * @since 2.5
		 */
		$screenshot = apply_filters( 'cozynest_sites_generate_child_screenshot', '' );
	 ...
	 */
	public function cozynest_sites_ajax_plugins() {

		if ( ! check_ajax_referer( 'cozynest_sites_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
			exit( 0 );
		}
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'cozynest' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'cozynest' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'cozynest' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			...
			$json['message'] = esc_html__( 'Installing', 'cozynest' );
			wp_send_json( $json );
		} else {
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'cozynest' ),
				)
			);
		}
	 ...
	 */
	public function cozynest_sites_ajax_content() {
		static $content = null;

		$selected_import = isset( $_POST['selected_index'] ) ? intval( $_POST['selected_index'] ) : false;

		if ( null === $content ) {
			$content = $this->get_import_data( $selected_import );
		}

		if ( ! check_ajax_referer( 'cozynest_sites_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'cozynest' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			...
			$json = array(
				...
				'action'         => 'cozynest_sites_content',
				'proceed'        => 'true',
				'content'        => sanitize_text_field( wp_unslash( $_POST['content'] ) ),
				'_wpnonce'       => wp_create_nonce( 'cozynest_sites_nonce' ),
		...
		if ( $json ) {
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'cozynest' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	 ...
	 */
	public function cozynest_sites_ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'cozynest_sites_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'cozynest' ),
				)
			);
		}
	...
	public function get_import_data_info( $selected_import_index = 0 ) {
		...
		if ( false !== has_action( 'cozynest_sites_after_all_import' ) ) {
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'cozynest' ),
				'description'      => esc_html__( 'Demo content data.', 'cozynest' ),
				'pending'          => esc_html__( 'Pending', 'cozynest' ),
				'installing'       => esc_html__( 'Installing', 'cozynest' ),
				'success'          => esc_html__( 'Success', 'cozynest' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'cozynest' ),
				'description'      => esc_html__( 'Sample widgets data.', 'cozynest' ),
				'pending'          => esc_html__( 'Pending', 'cozynest' ),
				'installing'       => esc_html__( 'Installing', 'cozynest' ),
				'success'          => esc_html__( 'Success', 'cozynest' ),
				'install_callback' => array( 'Cozynest_Sites_Widget_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'cozynest' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'cozynest' ),
				'pending'          => esc_html__( 'Pending', 'cozynest' ),
				'installing'       => esc_html__( 'Installing', 'cozynest' ),
				'success'          => esc_html__( 'Success', 'cozynest' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'cozynest' ),
				'description'      => esc_html__( 'Sample theme options data.', 'cozynest' ),
				'pending'          => esc_html__( 'Pending', 'cozynest' ),
				'installing'       => esc_html__( 'Installing', 'cozynest' ),
				'success'          => esc_html__( 'Success', 'cozynest' ),
				'install_callback' => array( 'Cozynest_Sites_Customizer_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'cozynest' ),
				'description'      => esc_html__( 'Redux framework options.', 'cozynest' ),
				'pending'          => esc_html__( 'Pending', 'cozynest' ),
				'installing'       => esc_html__( 'Installing', 'cozynest' ),
				'success'          => esc_html__( 'Success', 'cozynest' ),
				'install_callback' => array( 'Cozynest_Sites_Redux_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( false !== has_action( 'cozynest_sites_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'cozynest' ),
				'description'      => esc_html__( 'After import setup.', 'cozynest' ),
				'pending'          => esc_html__( 'Pending', 'cozynest' ),
				'installing'       => esc_html__( 'Installing', 'cozynest' ),
				'success'          => esc_html__( 'Success', 'cozynest' ),
		...
		/**
		 * Cozynest sites get base content
		 *
		 * @since 2.5
		 */
		$content = apply_filters( 'cozynest_sites_get_base_content', $content, $this );
	 ...
	 */
	public function cozynest_sites_pt_importer_new_ajax_request_response_data( $data ) {
		$data['url']      = admin_url( 'admin-ajax.php' );
		$data['message']  = esc_html__( 'Installing', 'cozynest' );
		$data['proceed']  = 'true';
		$data['action']   = 'cozynest_sites_content';
		$data['content']  = 'content';
		$data['_wpnonce'] = wp_create_nonce( 'cozynest_sites_nonce' );
	 ...
	 */
	public function cozynest_sites_after_content_import_setup( $selected_import_index ) {
		...
		// Get menus.
		$locations['primary']     = $this->cozynest_sites_get_menu_by_name( $selected_demo['primary_menu'] );
		$locations['vertical']    = $this->cozynest_sites_get_menu_by_name( $selected_demo['vertical_menu'] );
		$locations['footer_menu'] = $this->cozynest_sites_get_menu_by_name( $selected_demo['footer_menu'] );
		 ...
		 * @since 2.5
		 */
		$homepage = get_page_by_title( apply_filters( 'cozynest_sites_content_home_page_title', $selected_demo['homepage'] ) );
		 ...
		 * @since 2.5
		 */
		$blogpage = get_page_by_title( apply_filters( 'cozynest_sites_content_blog_page_title', $selected_demo['blog_page'] ) );
	 ...
	 */
	public function cozynest_sites_get_menu_by_name( $name ) {
	 ...
	 */
	public function cozynest_sites_before_content_import_setup() {
	...
	/**
	 * Register the import files via the `cozynest_sites_import_files` filter.
	 */
	public function cozynest_sites_register_import_files() {
		/**
		 * Cozynest sites import files
		 *
		 * @since 2.5
		 */
		$this->import_files = $this->validate_import_file_info( apply_filters( 'cozynest_sites_import_files', array() ) );
	...
	public function set_import_file_base_name() {
		$existing_name = get_transient( 'cozynest_sites_import_file_base_name' );
		...
		set_transient( 'cozynest_sites_import_file_base_name', $this->import_file_base_name, MINUTE_IN_SECONDS );
	}
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		$downloader = new Cozynest_Sites_Downloader();
	 ...
	 */
	public function cozynest_sites_selected_import_data_info() {
		// Nonce check.
		check_ajax_referer( 'cozynest_sites_selected_import_data_info', 'nonce' );
		...
		if ( 'pro' === $import_type && defined( 'COZYNEST_PRO_VERSION' ) ) {
			$modules_activated = get_option( 'cozynest_pro_fully_featured_activate' );
	 ...
	 * AJAX callback for the 'merlin_update_selected_import_data_info' action.
	 */
	public function cozynest_sites_update_selected_import_data_info() {
		check_ajax_referer( 'cozynest_sites_nonce' );
		$this->selected_index = ! isset( $_POST['selected_index'] ) ? false : intval( $_POST['selected_index'] );
	 ...
	 */
	public function cozynest_sites_import_finished() {
		delete_transient( 'cozynest_sites_import_file_base_name' );
	 ...
	 */
	public function cozynest_sites_register_required_plugins() {
		if ( isset( $_COOKIE['demo'] ) && null !== $_COOKIE['demo'] ) {
			$selected_import_index = sanitize_text_field( wp_unslash( $_COOKIE['demo'] ) );
			cozynest_site_plugin( $selected_import_index );
		}
	}

	/**
	 * Cozynest site filter demo
	 */
	public function cozynest_site_filter_demo() {
		check_ajax_referer( 'cozynest_sites_nonce' );
		$page_builder = ( isset( $_POST['page_builder'] ) ) ? sanitize_text_field( wp_unslash( $_POST['page_builder'] ) ) : 'elementor';
		...
		foreach ( $all_demo as $demo ) {
			if ( 'all' === $category || $demo['type'] === $category ) {
		...
		if ( ! empty( $demos ) ) {
			...
			foreach ( $demos[0] as $demo ) {
				$html .= $this->cozynest_demo_template( $demo );
			}
		} else {
			$html = '<h3 class="noting-found">' . esc_html__( 'Sorry! There is no demo for your choice.', 'cozynest' ) . '</h3>';
		}
	...
	/**
	 * Cozynest demo template
	 *
	 * @param  [type] $demo type.
	 */
	public function cozynest_demo_template( $demo ) {
		...
		$button           = '';
		if ( ! empty( $demo_image ) ) {
			$image_html = '<img src="' . esc_url( $demo_image ) . ' " alt="' . esc_attr_e( 'Demo Image', 'cozynest' ) . '">';
		}

		if ( 'pro' === $demo_type && defined( 'COZYNEST_PRO_VERSION' ) ) {
			$button = '<button data-content="' . esc_attr( $demo['id'] ) . '" class="merlin__demo-button js-select-demo">' . esc_html__( 'Select', 'cozynest' ) . '</button>';
		} elseif ( 'pro' === $demo_type && ! defined( 'COZYNEST_PRO_VERSION' ) ) {
			$button = '<a class="merlin__demo-button" href="https://cozynest.com/pro" target="_blank">' . esc_html__( 'Pricing', 'cozynest' ) . '</a>';
		} elseif ( 'free' === $demo_type ) {
			$button = '<button data-content="' . esc_attr( $demo['id'] ) . '" class="merlin__demo-button js-select-demo" data-callback="install_contents">' . esc_html__( 'Select', 'cozynest' ) . '</button>';
		}
		...
		$html = '<div class="merlin__demo">
					...
					<div class="merlin__demo-content">
						...
						<div class="merlin__demo-cta">
							<a class="merlin__demo-button" href="' . esc_url( $demo_preview_url ) . '" target="_blank">' . esc_html__( 'Preview', 'cozynest' ) . '</a>' .
							$button .
	...
	/**
	 * Cozynest sites load more demo
	 */
	public function cozynest_sites_load_more_demo() {
		check_ajax_referer( 'cozynest_sites_nonce' );
	...
	/**
	 * Cozynest sites start session
	 */
	public function cozynest_sites_start_session() {
		if ( ! session_id() ) {
			session_start();
		}
	...
	/**
	 * Cozynest sites set cookie
	 */
	public function cozynest_sites_set_cookie() {
		$total_page = 1;
		$demo       = 0;
		$files      = cozynest_sites_local_import_files();
		 ...
		 * @since 2.5
		 */
		$demos = apply_filters( 'cozynest_sites_import_files', $files );

		if ( ! headers_sent() ) {
			setcookie( 'total_page', $total_page, time() + 7200 );
			if ( count( $demos ) ) {
				setcookie( 'demo', $demo, time() + 7200 );
			}
		}
	}
	...
	public function setting_screen() {
		$demo_template = cozynest_sites_local_import_files();
		?>
		<div id="cozynest-site-template-page" class="cozynest-site-template-page">
			<div class="cozynest-template-wrapper">
				<div class="cozynest-header-sticky">
					<div class="header-sticky-wrapper">
						<div class="cozynest-main-content">
							<div class="back-to-layout" id="backtolayout">
								<span class="ion-ios-arrow-back back-icon"></span>
							</div>
							<div class="cozynest-action-content">
								<div class="cozynest-ajax-search">
									<div class="form-ajax-search">
										<div class="form-ajax-wrapper">
											<div class="filter-category">
												<div class="filter-category-wrapper">
													<span id="filter-category-selected" class="filter-category-selected"><?php echo esc_html__( 'All', 'cozynest' ); ?></span>
													<ul class="list-sort">
														<li class="sortby-item" slug="">
															<?php echo esc_html__( 'All', 'cozynest' ); ?>
														</li>
														<li class="sortby-item" slug="<?php echo esc_attr( 'free' ); ?>">
															<?php echo esc_html__( 'Free', 'cozynest' ); ?>
														</li>
														<li class="sortby-item" slug="<?php echo esc_attr( 'pro' ); ?>">
															<?php echo esc_html__( 'Pro', 'cozynest' ); ?>
														</li>
													</ul>
												</div>
											</div>
								...
								<div class="cozynest-favories-template">
				...
				<div class="cozynest-template-body">
					<div id="cozynest-demo-theme" class="cozynest-demo-theme cozynest-demo-template">
						<div class="list-demo-template">
							<?php foreach ( $demo_template as $demo ) : ?>
								<div class="template-item" demo-id="<?php echo esc_attr( $demo['id'] ); ?>">
									<div class="item-template-wrapper">
										<div class="template-image">
											...
											<?php if ( 'pro' == $demo['type'] ) : ?>
												<span class="pro-lable"><?php echo esc_html__( 'Pro', 'cozynest' ); ?></span>
											<?php endif ?>
										</div>
	...
	/**
	 * Cozynest sites admin template
	 */
	public function cozynest_sites_admin_template() {
		wp_enqueue_style(
			'cozynest_admin_site_template',
			get_template_directory_uri() . '/inc/cozynest-sites-library/assets/css/admin.css',
			array( 'wp-admin' ),
			COZYNEST_SITES_VER
		);

		wp_enqueue_script(
			'cozynest-admin-template-scripts',
			get_template_directory_uri() . '/inc/cozynest-sites-library/assets/js/admin.js',
			array( 'jquery-core' ),
			COZYNEST_SITES_VER,
			true
		);
		...
		$admin_vars = array(
			'url'          => admin_url( 'admin-ajax.php' ),
			'nonce'        => wp_create_nonce( 'cozynest_admin_template_nonce' ),
			'redirect_url' => '/',
		);
	 ...
	 */
	public function cozynest_ajax_all_feature_activated() {
		/*Bail if the nonce doesn't check out*/
		if ( ! current_user_can( 'update_plugins' ) ) {
			return;
		}

		$current = get_option( 'cozynest_pro_fully_featured_activate' );

		/*Do another nonce check*/
		check_ajax_referer( 'cozynest_sites_nonce', 'ajax_nonce' );
		$detect = isset( $_POST['detect'] ) ? sanitize_text_field( wp_unslash( $_POST['detect'] ) ) : '';

		if ( $detect !== $current ) {
			update_option( 'cozynest_pro_fully_featured_activate', $detect );
		}
		$response['fully_featured_activate'] = get_option( 'cozynest_pro_fully_featured_activate' );
	 ...
	 */
	public function cozynest_ajax_module_action() {
		check_ajax_referer( 'cozynest_sites_nonce', 'ajax_nonce' );
		...
		if ( isset( $_POST['name'] ) ) {
			...
			if ( isset( $_POST['counter'] ) ) {
				$response['index'] = intval( $_POST['counter'] );
			}
