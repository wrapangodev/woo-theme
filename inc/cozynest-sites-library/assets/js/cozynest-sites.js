...
var Cozynest_Sites = (function($){
	...
	function window_loaded(){
		...
		btnLoadMore.on('click', function(e) {
			var pageBuilder = $('.merlin__other-page-builder').find('.active').attr('data-group'),
				...
				data = {
					action: 'cozynest_sites_load_more_demo',
					_ajax_nonce: cozynest_sites_params.wpnonce,
					page: page,
					category: cat,
			...
			$.ajax({ // you can also use $.post here
				url : cozynest_sites_params.ajaxurl, // AJAX handler
				data : data,
				type : 'POST',
				...
				success : function( response ){
					...
					if( response ) {
						...
						if ( page >= cozynest_sites_params.total_page ) {
		...
		btn_filter_link.on( 'click', function (e) {
			...
			var pageBuilder = $('.merlin__other-page-builder').find('.active').attr('data-group'),
				cat = $('.merlin__categories').find('.active').attr('data-group');
			var data = {
					action: 'cozynest_site_filter_demo',
					_ajax_nonce: cozynest_sites_params.wpnonce,
					page_builder: pageBuilder,
					category: cat,
				};
		...
		$('.merlin__demos ').on( 'click', '.js-select-demo', function() {
			...
			$.post( cozynest_sites_params.ajaxurl, {
				action: 'cozynest_sites_update_selected_import_data_info',
				_ajax_nonce: cozynest_sites_params.wpnonce,
				...
				.fail( function() { alert( cozynest_sites_params.texts.something_went_wrong ); } );
		} );
	...
	function ChildTheme() {
		...
		function do_ajax() {
			jQuery.post(cozynest_sites_params.ajaxurl, {
				action: "cozynest_sites_child_theme",
				wpnonce: cozynest_sites_params.wpnonce,
	...
	function ActivateLicense() {
		...
		function do_ajax() {
			...
			jQuery.post(cozynest_sites_params.ajaxurl, {
			  action: "cozynest_sites_activate_license",
			  wpnonce: cozynest_sites_params.wpnonce,
	...
	function ContentManager(){
		...
		function init_content_import_progress_bar() {
			...
			jQuery.post(cozynest_sites_params.ajaxurl, {
				action: "cozynest_sites_get_total_content_import_items",
				wpnonce: cozynest_sites_params.wpnonce,
				selected_index: getCookie('demo'),
			}, function( response ) {
		...
		return {
			...
			$.post(cozynest_sites_params.ajaxurl, {
				action: "cozynest_sites_import_finished",
				wpnonce: cozynest_sites_params.wpnonce,
				selected_index: getCookie('demo'),
			});
	...
	function ModuleManager(){
		...
		function ajax_active( module_id, item ) {
			var data     = {
				action: 'cozynest_sites_module_action',
				ajax_nonce: cozynest_sites_params.wpnonce,
				name: module_id,
				counter: counter
			};
			...
			$.ajax(
				{
					type: 'POST',
					url: cozynest_sites_params.ajaxurl,
					data: data,
